...
class Paths
{
	...
	public static function getPath(file:String, ?type:AssetType = TEXT, ?parentfolder:String, ?modsAllowed:Bool = true):String
	{
		...
		if(modsAllowed)
		{
			...
			if(OpenFlAssets.exists(modded)) return modded;
		}
		#end
	...
	public static function fileExists(key:String, type:AssetType, ?ignoreMods:Bool = false, ?parentFolder:String = null)
	{
		...
		if(!ignoreMods)
		{
			...
			for(mod in Mods.getGlobalMods())
				if (OpenFlAssets.exists(mods('$mod/$modKey')))
					return true;

			if (OpenFlAssets.exists(mods(Mods.currentModDirectory + '/' + modKey)) || OpenFlAssets.exists(mods(modKey)))
				return true;
		}
		#end
	...
	static public function getAtlas(key:String, ?parentFolder:String = null, ?allowGPU:Bool = true):FlxAtlasFrames
	{
		...
		if(OpenFlAssets.exists(myXml) #if !cpp || (OpenFlAssets.exists(myXml) && (useMod = true)) #end )
		{
			#if !cpp
			return FlxAtlasFrames.fromSparrow(imageLoaded, (useMod ? Assets.getText(myXml) : myXml));
			#else
		...
		else
		{
			...
			if(OpenFlAssets.exists(myJson) #if !cpp || (OpenFlAssets.exists(myJson) && (useMod = true)) #end )
			{
				#if !cpp
				return FlxAtlasFrames.fromTexturePackerJson(imageLoaded, (useMod ? Assets.getText(myJson) : myJson));
				#else
	...
	inline static public function getSparrowAtlas(key:String, ?parentFolder:String = null, ?allowGPU:Bool = true):FlxAtlasFrames
	{
		...
		if(OpenFlAssets.exists(xml)) xmlExists = true;

		return FlxAtlasFrames.fromSparrow(imageLoaded, (xmlExists ? Assets.getText(xml) : getPath(Language.getFileTranslation('images/$key') + '.xml', TEXT, parentFolder)));
		#else
	...
	inline static public function getPackerAtlas(key:String, ?parentFolder:String = null, ?allowGPU:Bool = true):FlxAtlasFrames
	{
		...
		if(OpenFlAssets.exists(txt)) txtExists = true;

		return FlxAtlasFrames.fromSpriteSheetPacker(imageLoaded, (txtExists ? Assets.getText(txt) : getPath(Language.getFileTranslation('images/$key') + '.txt', TEXT, parentFolder)));
		#else
	...
	inline static public function getAsepriteAtlas(key:String, ?parentFolder:String = null, ?allowGPU:Bool = true):FlxAtlasFrames
	{
		...
		if(OpenFlAssets.exists(json)) jsonExists = true;

		return FlxAtlasFrames.fromTexturePackerJson(imageLoaded, (jsonExists ? Assets.getText(json) : getPath(Language.getFileTranslation('images/$key') + '.json', TEXT, parentFolder)));
		#else
